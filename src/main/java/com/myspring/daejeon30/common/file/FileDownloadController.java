...
public class FileDownloadController {
	...
	@RequestMapping("/downloadreview.do")//바로 아래 행에서 이미지 파일 이름을 바로 설정합니다.
	public void downloadreview(@RequestParam("imageFileName") String imageFileName, 
						 @RequestParam("num") String num,
						 HttpServletResponse response) throws Exception {
		OutputStream out = response.getOutputStream();
		String downFile = REVIEW_IMAGE_REPO + "\\"+num + "\\" + imageFileName; //글 번호와 파일 이름으로 다운로드할 파일 경로를 설정합니다.
		File file = new File(downFile);
